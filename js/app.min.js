...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var inputFieldVisibilityBtn = document.querySelectorAll('.input-field__visibility-btn');
  if (inputFieldVisibilityBtn) {
    inputFieldVisibilityBtn.forEach(function (item) {
      item.addEventListener('click', function (event) {
        event.preventDefault();
        var parent = item.closest('.input-field');
  ...
  function headerFixedDown() {
    if (window.pageYOffset >= header.getBoundingClientRect().height) {
      ...
      header.querySelector('.header__fixed').style.transition = 'transform 0.2s ease-in-out';
      setTimeout(function () {
        return header.querySelector('.header__fixed').style.position = 'fixed';
      }, 200);
    }
  }
