...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function reload(item) {
    ...
    item.addEventListener('mouseenter', function () {
      ...
      ideasBullets.forEach(function (child) {
        return child.removeAttribute('data-current');
      });
      item.dataset.current = true;
      ideasImg.forEach(function (child) {
        return child.removeAttribute('data-current');
      });
    ...
    catalogFilterCheckboxTrigger.forEach(function (item) {
      item.addEventListener('click', function (event) {
        ...
        if (!item.closest('.catalog-filter__checkbox-group').classList.contains('is--open')) {
          slideDownQna(item.parentNode.nextElementSibling);
        } else {
          slideUpQna(item.parentNode.nextElementSibling);
        }
      });
    });
  }
            ...
            if (item.classList.contains('catalog-filter__checkbox--all')) {
              if (input.checked) {
                ...
                if (parent.classList.contains('is--open')) {
                  slideUpQna(parent.querySelector('.catalog-filter__checkbox-dropdown'));
                }
              } else {
                input.checked = true;
                parent.querySelectorAll('.catalog-filter__checkbox:not(.catalog-filter__checkbox--all) input[type=checkbox]').forEach(function (child) {
                ...
                if (!parent.classList.contains('is--open')) {
                  slideDownQna(parent.querySelector('.catalog-filter__checkbox-dropdown'));
                }
              }
            } else {
              if (input.checked) {
  ...
  if (ideasImg && ideasBullets) {
    ideasBullets.forEach(function (item) {
      reload(item);
    });
    if (ideasTooltips) {
      ideasTooltips.forEach(function (ideasTooltip) {
        ideasTooltip.addEventListener('mouseover', function () {
          ideasTooltip.style.display = 'block';
        ...
        ideasTooltip.addEventListener('mouseout', function () {
          ideasTooltip.style.position = 'absolute';
          ideasTooltip.style.display = 'none';
          ideasBullets.forEach(function (child) {
    ...
    document.body.addEventListener('mouseover', function (event) {
      var ideasTooltip = event.target.closest('.ideas-tooltip');
      if (ideasTooltip) {
        ideasTooltip.style.display = 'block';
        document.querySelector('.ideas__img-bullet[data-current="true"]').classList.add('is--hover');
        document.querySelector('.ideas__img[data-current="true"]').classList.add('is--hover');
  ...
  var ideasPopupTriggerMy = document.querySelectorAll('.js-ideas-gallery');
  var ideasPopupClose = document.querySelectorAll('.ideas-popup__close');
  if (ideasPopup && ideasPopupTrigger && ideasPopupTriggerMy && ideasPopupClose) {
    ...
    ideasPopupTrigger.forEach(function (item, i) {
      item.addEventListener('click', function (event) {
        ...
        if (!event.target.classList.contains('ideas__img-bullet') || !event.target.closest('.ideas__img-bullet')) {
          event.preventDefault();
          var url = item.querySelector('.js-ideas-gallery').dataset.url;
          var el = item.querySelector('.js-ideas-gallery').dataset.el;
          $.ajax({
            url: url,
            success: function success(data) {
      ...
      //     if (event.target.classList.contains('ideas-popup-trigger') || event.target.parentNode.classList.contains('ideas__img-icon') || event.target.classList.contains('ideas__img-value')) {
      //         event.preventDefault();
    ...
    // ideasPopupTriggerMy.forEach(function(item, i) {
    //     item.addEventListener('click', function(event) {
    //         if (item.dataset.url) {
    //             event.preventDefault();
    ...
    //                     // ideas popup slider
    //                     const ideasPopupSlider = document.querySelector('#ideas-popup__slider-wrapper .swiper');
    ...
    //                     const newImgBullets = document.querySelectorAll('#ideas-popup__slider-wrapper a.ideas__img-bullet');
